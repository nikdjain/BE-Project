...
def read_next_data():
    ...
    def show():
        ...
        pin_num =pin_number.get()
        p = int(p)
        pin_num = int(pin_num)
        if(p == OTP and pin_num == pin_num_from_database):
            ...
            def pin_change():
                ...
                for record in records:
                    if int(record[0]) == ID+1:
                        phone_numberr = record[2]
                        pin_num_from_database = record[4]
                        break
                ...
                def db_update():
                    ...
                    if(OTP == int(o) and int(p1) == int(p2)) :
                        mydb = mysql.connector.connect(host='localhost',
                                database='Multi_Bank',
                                user='root',
                                password='king@rO7',auth_plugin='mysql_native_password')
                        ...
                        mycursor = mydb.cursor()
                        sql = "UPDATE user_info SET pswd = %s WHERE id = %s"
                        val = (str(p1) , str(ID+1))
                        mycursor.execute(sql, val)
                        mydb.commit()
                        pin.destroy()
                ...
                pin = Tk()
                pin.geometry("1300x600+30+30")
                pin.configure(background=colour_f)

                label2 = Label(pin, text="COLLEGE  BANK  OF  INDIA")
                label2.configure(background=colour_d)
                label2.configure(foreground=colour_w)
                label2.config(font=("Times New Roman", 40))
                label2.place(x = 0,y=0,height=75, width=1300)

                label1= Label(pin, text="Change Pin")
                label1.configure(background=colour_f)
                label1.config(font=("Courier",25))
                label1.place(x = 350,y=120,height=70, width=600)
##
                label1 = Label(pin, text="OTP")
                label1.configure(background=colour_f)
                label1.configure(foreground=colour_d)
                label1.config(font=("Timesnew roman",14))
                label1.place(x = 350,y=200,height=30, width=300)
                
                otp = StringVar() 
                passEntry = Entry(pin, textvariable=otp)
...
##
                label1 = Label(pin, text="Password")
                label1.configure(background=colour_f)
                label1.configure(foreground=colour_d)
                label1.config(font=("Timesnew roman",14))
                label1.place(x = 350,y=250,height=30, width=300)

                pin1 = StringVar() 
                passEntry = Entry(pin, textvariable=pin1, show="*")
                passEntry.place(x = 650,y=250,height=30, width=200)
                passEntry.config(font=("Timesnew roman",14))
                passEntry.configure(background=colour_w)
##
                label1 = Label(pin, text="Confirm Password")
                label1.configure(background=colour_f)
                label1.configure(foreground=colour_d)
                label1.config(font=("Timesnew roman",14))
                label1.place(x = 350,y=300,height=30, width=300)

                pin2 = StringVar() 
                passEntry = Entry(pin, textvariable=pin2, show="*")
                passEntry.place(x = 650,y=300,height=30, width=200)
                passEntry.config(font=("Timesnew roman",14))
                passEntry.configure(background=colour_w)
##
                B1 = Button(pin, text = "Save", command = db_update)
            ...
            B1 = Button(bank_choice, text = "Change PIN", command = pin_change)
            ...
            B1 = Button(bank_choice, text = "EXIT", command = Exit)
...
##    send_OTP(OTP)
    ...
    for record in records:
        if int(record[0]) == ID+1:
            phone_numberr = record[2]
            pin_num_from_database = int(record[4])
            break
    ...
    if(face_id_val == ID):
        ...
        sql_select_Query = "select * from user_info WHERE id = " + str(ID + 1)
...
##
        ...
        label3 = Label(top, text="Enter 4 digit pin ")
        label3.configure(background=colour_f)
        label3.configure(foreground=colour_d)
        label3.config(font=("Courier",15))
        label3.place(x = 350,y=220,height=70, width=600)

        pin_number = StringVar() #Password variable
        passEntry = Entry(top, textvariable=pin_number, show='*')
        passEntry.place(x = 550,y=270,height=30, width=200)
        passEntry.configure(background=colour_w)
##
        label3 = Label(top, text="Enter OTP send to your registered Mobile Number")
        label3.configure(background=colour_f)
        label3.configure(foreground=colour_d)
        label3.config(font=("Courier",15))
        label3.place(x = 350,y=350,height=70, width=600)

        password = StringVar() #Password variable
        passEntry = Entry(top, textvariable=password, show='*')
        passEntry.place(x = 550,y=400,height=30, width=200)
        passEntry.configure(background=colour_w)
    ##
        submit = Button(top, text='ENTER',command=show)
        submit.place(x = 575,y = 500 ,height=30, width=150)
